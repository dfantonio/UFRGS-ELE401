...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define PARAMS 31 // Quantidade para o modelo baseline
// #define PARAMS_SIZE (PARAMS * sizeof(float)) // Tamanho do buffer necessário para receber floats (baseline)
#define PARAMS 4                              // Quantidade de parâmetros do modelo convertido
#define PARAMS_SIZE (PARAMS * sizeof(int8_t)) // Tamanho do buffer necessário para receber inteiros (modelo convertido)
/* USER CODE END PD */
...
int main(void)
{
  ...
  int8_t decoded_ints[PARAMS];  // Array to store the decoded integers
  char tx_buff[20] = {0};
  ...
  while (1)
  {
    if (HAL_UART_Receive(&huart2, (uint8_t *)rx_buff, PARAMS_SIZE, 1000) == HAL_OK)
    {
      ...
      for (int i = 0; i < PARAMS; i++)
      {
        ...
        memcpy(&decoded_ints[i], &rx_buff[i * sizeof(int8_t)], sizeof(int8_t));
      }
      ...
      int resultado = modelo_convertido_predict(decoded_ints, PARAMS);
